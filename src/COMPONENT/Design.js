...
const Design = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`http://localhost:3001/design/getCategoryone`, {
        params: { roomType_id },
      })
      .then((response) => {
          ...
          <div className="row ms-5">
            {designs.map((design, index) => (
              <div className="col-md-4 p-3" key={index}>
                ...
                >
                  <div
                    className="designs-img d-flex m-0 "
  ...
  design.design_id ? navigate(`/particular/${design.design_id}`, { state: { designs, design } }) : alert('Design ID is undefined')
                      ...
                      <button
                        ...
                        onClick={() =>
                          design.designImg_id
                            ? navigate(`/particular/${design.designImg_id}`, {
                                state: { designs, design },
                              })
                            : alert("Design ID is undefined" + design.designImg_id)
                        }
                      >
                        View More
                      </button>
